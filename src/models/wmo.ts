const WMO_CODE_MAP: { [key: string]: { [key: string]: {description: string, image: string} } } = {
    "0": {
        "day": {
            "description": "Sunny",
...
function unknown_code(code: number): WeatherCodeInfo {
    return {
        code,
        description: `Unknown Code ${code}`,
        image_url: "https://openweathermap.org/img/wn/50n@2x.png"
    };
}

export function get_weather_code_info(code: number, time_of_day: "day"|"night"): WeatherCodeInfo {
    const code_key = code.toString();
    if (code_key in WMO_CODE_MAP) {
        const code_info = WMO_CODE_MAP[code_key][time_of_day];
        return {
            code,
            description: code_info.description,
            image_url: code_info.image
        };
    } else {
